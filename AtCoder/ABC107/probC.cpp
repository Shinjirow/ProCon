...
signed main(){
    ...
    rep(i,n-k+1){
        //show(vec[i],vec[i+k-1]);
        if((vec[i] xor vec[i+k-1])<0){ //異符号
            ans=min({ans,abs(vec[i+k-1]*2+abs(vec[i])),abs(vec[i+k-1])+abs(vec[i])*2});
        }else{
            ans=min(ans,max(abs(vec[i]),abs(vec[i+k-1])));
        }
    }
